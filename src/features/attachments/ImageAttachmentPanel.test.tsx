...
describe('ImageAttachmentPanel', () => {
  ...
  it('creates local image attachments from selected files', async () => {
    const onAdd = vi.fn<(attachment: Attachment) => void>();
    vi.spyOn(URL, 'createObjectURL').mockReturnValue('blob:image');
    render(<ImageAttachmentPanel attachments={[]} existingBytes={0} noteId="note-1" onAdd={onAdd} onRemove={vi.fn()} />);
  ...
  it('renders persisted images and delegates removal', () => {
    ...
    render(<ImageAttachmentPanel attachments={[attachment]} existingBytes={0} noteId="note-2" onAdd={vi.fn()} onRemove={onRemove} />);
